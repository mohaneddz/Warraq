    ...
    <div className="flex min-h-screen items-center justify-center bg-[#F9F8F4] dark:bg-[#111d1a] px-4 py-8">
      <div className="max-w-2xl w-full bg-white dark:bg-[#1d2926] p-8 md:p-10 rounded-2xl border border-black/5 dark:border-white/5 shadow-card flex flex-col">
        ...
        <div className="flex items-center gap-3 mb-8 pb-6 border-b border-black/5 dark:border-white/5">
          <div className="w-12 h-12 bg-[#1a4d40] rounded-xl flex items-center justify-center shadow-inner">
            <img src="/brand/warraq-symbol-cream.png" className="h-8 w-8 object-contain" alt="" />
          </div>
          <div>
            <strong className="block text-[18px] font-bold text-[#122222] dark:text-white tracking-widest uppercase">WARRAQ</strong>
        ...
        {/* Title */}
        <div className="mb-8">
          <h1 className="font-display text-[26px] font-bold text-[#122222] dark:text-white leading-tight">Welcome to your workspace</h1>
          <p className="text-[14px] text-[#122222]/60 dark:text-white/60 mt-1">Configure your local-first hospital library to get started.</p>
        </div>
        ...
        <div className="space-y-5 flex-1">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <label className="text-[11px] font-bold text-[#122222]/60 dark:text-white/60 uppercase tracking-wider block font-semibold">
              <span>Library name <span className="text-red-500">*</span></span>
              <input 
                type="text" 
                value={libName}
                onChange={(e) => setLibName(e.target.value)}
                placeholder="e.g. Mustapha Bacha Hospital Library" 
                className="w-full bg-[#fcfbf8] dark:bg-[#111d1a] border border-black/10 dark:border-white/10 rounded-xl py-3 px-4 text-[13px] text-[#122222] dark:text-white outline-none focus:border-[#1a4d40] mt-1.5 font-semibold"
                required
              />
            </label>
            <label className="text-[11px] font-bold text-[#122222]/60 dark:text-white/60 uppercase tracking-wider block font-semibold">
              <span>Librarian / Operator name <span className="text-red-500">*</span></span>
              <input 
                type="text" 
                value={operator}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div>
              <span className="text-[11px] font-bold text-[#122222]/60 dark:text-white/60 uppercase tracking-wider block mb-1.5 font-semibold">Theme preference</span>
          ...
          <div className="pt-2">
            <label className="flex items-start gap-3 cursor-pointer select-none p-4 rounded-xl border border-black/5 dark:border-white/5 bg-[#fcfbf8] dark:bg-[#111d1a] hover:bg-black/5 dark:hover:bg-white/5 transition-colors font-semibold">
              ...
              />
              <div>
                <strong className="block text-[13px] text-[#122222] dark:text-white font-bold">Enable optional circulation fines</strong>
                <small className="block text-[11px] text-[#122222]/50 dark:text-white/50 mt-0.5 font-normal">Track overdue fees to encourage timely book returns.</small>
              </div>
            </label>
          </div>
        </div>
        ...
        <div className="mt-10 pt-6 border-t border-black/5 dark:border-white/5 flex flex-col gap-4">
          <button 
            ...
            className="bg-[#1a4d40] text-white w-full py-4 rounded-xl font-bold text-[15px] hover:bg-[#1a4d40]/90 transition-colors shadow-sm disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
          >
            Start using Warraq <span>→</span>
          </button>
          <p className="flex items-center justify-center gap-1.5 text-[11px] text-[#122222]/50 dark:text-white/50">
            <ShieldCheck size={14} className="text-emerald-600" /> Your library data stays secure and private on this device.
          </p>
        </div>
      </div>
    </div>
