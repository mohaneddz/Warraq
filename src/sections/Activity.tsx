  ...
  return (
    <div className="flex flex-col h-full w-full">
      ...
      <div className="flex justify-between items-end mb-8">
        <div>
          <h1 className="font-display text-[28px] font-bold text-[#122222] dark:text-white leading-tight">Activity</h1>
          <p className="text-[13px] text-[#122222]/60 dark:text-white/60">Immutable local audit history for important library actions.</p>
        </div>
        <div className="flex items-center gap-3">
          <button 
            onClick={handleExportCSV}
            className="flex items-center gap-2 bg-white dark:bg-[#1d2926] border border-black/10 dark:border-white/10 text-[#122222] dark:text-white px-4 py-2 rounded-lg font-bold text-[13px] hover:bg-black/5 dark:hover:bg-white/5 transition-colors shadow-card"
          >
        ...
        <div className="p-4 border-b border-black/5 dark:border-white/5 flex items-center gap-3 bg-[#fcfbf8] dark:bg-[#111d1a] flex-wrap md:flex-nowrap">
          <div className="flex-1 max-w-sm relative">
            ...
            <input 
              ...
              className="w-full bg-white dark:bg-[#1d2926] border border-black/10 dark:border-white/10 rounded-lg py-2 pl-9 pr-3 text-[13px] text-[#122222] dark:text-[#f0ebe1] outline-none focus:border-[#1a4d40]" 
            />
          </div>
          ...
          <div className="flex items-center gap-2 bg-white dark:bg-[#1d2926] border border-black/5 dark:border-white/5 rounded-lg py-1.5 px-3">
             ...
             />
             {dateFilter && (
               <button onClick={() => setDateFilter("")} className="text-red-500 hover:text-red-700 ml-1 text-xs">Clear</button>
             )}
          </div>
          ...
          {result.isLoading ? (
            ...
            <table className="w-full text-left text-[13px]">
                ...
                {filteredLogs.map((item) => (
                  <tr key={item.id} className="hover:bg-black/5 dark:hover:bg-white/5 transition-colors group">
                    <td className="px-6 py-3">
                      <div className="font-semibold text-[#122222] dark:text-white">{new Date(item.created_at).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</div>
                      <div className="text-[11px] text-[#122222]/50 dark:text-white/50 mt-0.5">{new Date(item.created_at).toLocaleDateString()}</div>
                    </td>
                    <td className="px-6 py-3">
                      <div className="flex items-center gap-2">
                        <div className="w-6 h-6 rounded-full bg-[#122222] dark:bg-white/10 text-white flex items-center justify-center text-[10px] font-bold">
                    ...
                    <td className="px-6 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[11px] font-mono font-bold text-[#1a4d40] dark:text-[#1b9277] bg-[#1a4d40]/10 dark:bg-[#1b9277]/10 px-1.5 py-0.5 rounded">
             ...
             <div className="flex-1 flex flex-col items-center justify-center py-20 opacity-60">
              ...
              <h2 className="text-[20px] font-bold text-[#122222] dark:text-white mb-2">No activity matches filters</h2>
              <p className="text-[14px] text-[#122222]/60 dark:text-white/60">Try clearing filters or search term.</p>
            </div>
          )}
        </div>
      </div>
